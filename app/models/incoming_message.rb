...
class IncomingMessage < ActiveRecord::Base
    ...
    def mask_special_emails!(text)
        ...
        if self.info_request.public_body.is_followupable?
            text.gsub!(self.info_request.public_body.request_email, "[ correo para pedidos en " + self.info_request.public_body.short_or_long_name + " ]")
        end
        text.gsub!(self.info_request.incoming_email, "[FOI #" + self.info_request.id.to_s + " email]")
        text.gsub!(MySociety::Config.get("CONTACT_EMAIL", 'contact@localhost'), "[correo de contacto de #{MySociety::Config.get('SITE_NAME', 'Alaveteli')}]")
    end
